...
if 'df' not in locals().keys():
    ...
    tok_count = Counter()
    for i, ct in enumerate(is_df['clean_title'].values):
        if ct=='blank':
            ct = ''
    ...
    for row, ct in enumerate(is_df['clean_title'].values):
    ...
    hy_test = oos_df['int_rate']>12
    hy_train = is_df['int_rate']>12
...
json.dump(save_dict, open('logistic_regression_clean_title_log_odds.json','w'))
